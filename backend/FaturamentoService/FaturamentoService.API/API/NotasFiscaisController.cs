...
namespace FaturamentoService.API.API
{
    ...
    public sealed class NotasFiscaisController : ControllerBase
    {
        ...
        public NotasFiscaisController(ICriarNotaFiscalUseCase criar, IImprimirNotaFiscalUseCase imprimir)
        {
            _criar = criar;
            _imprimir = imprimir;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Criar([FromBody] CriarNotaFiscalEntradaDto entrada)
        {
            var resultado = await _criar.ExecutarAsync(entrada);
        ...
        public async Task<IActionResult> Imprimir([FromRoute] Guid id)
        {
            var resultado = await _imprimir.ExecutarAsync(id);
        ...
        private IActionResult ConverterFalhaCriacao<T> (Resultado<T> resultado)
        {
            ...
            return erro.Codigo switch
            {
                ...
                CodigoErro.Conflito => BadRequest(resultado.Erros), _ => BadRequest(resultado.Erros)
            };
        }
        ...
        private IActionResult ConverterFalhaImpressao<T> (Resultado<T> resultado)
        {
            ...
            return erro.Codigo switch
            {
                ...
                CodigoErro.Validacao => BadRequest(resultado.Erros), _ => BadRequest(resultado.Erros)
            };
        }
    }
}
